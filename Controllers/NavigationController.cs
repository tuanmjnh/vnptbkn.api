...
namespace VNPTBKN.API.Controllers
{
    ...
    public class NavigationController : Controller
    {
        ...
        public IActionResult ExistCode(string code)
        {
            try
            {
                if (db.Connection().isExist("Navigation", "code", code)) return Json(new { msg = TM.Core.Common.Message.exist.ToString() });
        ...
        public async Task<IActionResult> Post([FromBody] Models.Core.Navigation data)
        {
            try
            {
                ...
                if (db.Connection().isExist("Navigation", "code", data.code)) return Json(new { msg = TM.Core.Common.Message.exist.ToString() });
        ...
        public async Task<IActionResult> Delete([FromBody] List<dynamic> data)
        {
            try
            {
                ...
                foreach (var item in data)
                    qry += $"update Navigation set flag={item.flag},deleted_by='{nd.ma_nd}',deleted_at={DateTime.Now.ParseDateTime()} where id='{item.id}';\r\n";
                qry += "END;";
